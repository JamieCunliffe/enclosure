...
END-SYM The closing symbol to look for"
  (let* ((ctr 0)
         ...
         (end 0))
    (while (not
            (and (looking-at beg-sym)
                 (or
                  (= ctr 0)
                  (and
                   (= ctr 1)
                   (cl-search end-sym beg-sym)))))
...
(defun enclosure-change()
  ...
  (save-excursion
    (let* ((change-pair (enclosure--get-change-string))
           ...
           (beg-pair (enclosure--get-pair insert-char))
           (beg-symbol (if pair (plist-get pair :beginning) delete-char))
           (end-symbol (if pair (plist-get pair :end) delete-char))
           (new-beg-symbol (if beg-pair (plist-get beg-pair :end) insert-char))
           (beg-len (length beg-symbol))
           (end-len (length end-symbol))
           (new-beg-len (length new-beg-symbol))
           (position (enclosure--find-str-pos-for-pair beg-symbol end-symbol))
           (beg (car position))
           (end (car (cdr position))))
      (goto-char (- end end-len))
      ...
      (goto-char beg)
      (delete-char beg-len)
      (enclosure--add-chars beg (- end 1 beg-len (- beg-len new-beg-len) (- end-len beg-len)) insert-char))))
